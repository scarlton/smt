...
DEVELOPER_KEY = "AIzaSyAVivrRDWmvdcNmPkn2BZ2pQzDUjX3VUgk"
...
class Main(BaseRequestHandler):
    def get(self):
        # Render the template
        self.render("index.html", {"active_tab": "home"})


class Collections(BaseRequestHandler):
    def get(self):
        self.render("collections.html", {"active_tab": "collections"})

class NewCollection(BaseRequestHandler):
    def get(self):
        self.render("new-collection.html", {"active_tab": "collections"})

    def post(self):
        video_id = decode(self.request.get("videoId"))
        #insert video into task queue for collection
        self.redirect("/collections")

class Contacts(BaseRequestHandler):
    def get(self):
        self.render("contacts.html", {"active_tab": "contacts"})

class Messages(BaseRequestHandler):
    def get(self):
        self.render("messages.html", {"active_tab": "messages"})

class Metrics(BaseRequestHandler):
    def get(self):
        self.render("metrics.html", {"active_tab": "metrics"})
